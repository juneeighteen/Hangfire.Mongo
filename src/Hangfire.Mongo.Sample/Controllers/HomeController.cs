...
namespace Hangfire.Mongo.Sample.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult FireAndForget(int id)
        {
            for (int i = 0; i < id; i++)
            {
                BackgroundJob.Enqueue(() => Debug.WriteLine($"Hangfire fire-and-forget task started with id {i}."));
            }
        ...
        public ActionResult Delayed(int id)
        {
            for (int i = 0; i < id; i++)
            {
                BackgroundJob.Schedule(() => Debug.WriteLine($"Hangfire delayed task with id {i} started!"), TimeSpan.FromMinutes(1));
            }
